# -*- coding:utf-8 -*-
#@Time : 2021-05-26 20:53
#@Author: zxf_要努力
#@File : 贪心算法_452_用最少数量的箭引爆气球.py
'''
...
输入：points = [[1,2]]
输出：1
这题和435 差不多的意思 就先提出重叠区间 然后剩下的需要用弓射
'''
...
class Solution:
    def findMinArrowShots(self, points: List[List[int]]) -> int:
        if len(points) == 1:
            return 1
        sort_list = sorted(points,key=lambda x:x[1],reverse=False)
        result = list()
        result.append(sort_list.pop(0))
        while len(sort_list):
            if sort_list[0][0] <= result[-1][1]:
                sort_list.pop(0)
            else:
                result.append(sort_list.pop(0))
        return len(result)
